...
def analyze_vnstock_data():
    ...
    try:
        ...
        if not df_intraday.empty:
            print(f"✅ stock_intraday_data: {len(df_intraday)} records")
    ...
    try:
        ...
        if not df_hist.empty:
            print(f"✅ stock_historical_data: {len(df_hist)} records")
    ...
    try:
        ...
        if not df_price_board.empty:
            print(f"✅ price_board: {len(df_price_board)} records")
    ...
    try:
        ...
        if not df_companies.empty:
            print(f"✅ listing_companies: {len(df_companies)} records")
    ...
    try:
        ...
        if not df_indices.empty:
            print(f"✅ indices_listing: {len(df_indices)} records")
    ...
    try:
        ...
        if not df_top_movers.empty:
            print(f"✅ market_top_mover: {len(df_top_movers)} records")
            print(f"   Columns: {list(df_top_movers.columns)}")
            print(f"   Sample data:")
            print(df_top_movers.head(3).to_string())
        else:
            print("❌ No top movers data available")
    except Exception as e:
        print(f"❌ Error: {e}")
    ...
    try:
        ...
        if not df_financial.empty:
            print(f"✅ financial_ratio: {len(df_financial)} records")
    ...
    try:
        ...
        if not df_profile.empty:
            print(f"✅ company_profile: {len(df_profile)} records")
...
🗄️ POSTGRESQL TABLES RECOMMENDATION:
